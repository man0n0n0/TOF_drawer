...
class Stepper:
    ...
    def overwrite_pos(self, p):
        self.pos = p  # Fixed: was setting to 0 instead of p
    ...
    def step(self, d):
        ...
        t1: ENA must be ahead of DIR by at least 5μs
        t2: DIR must be ahead of PUL by 5μs  
        t3: Pulse width not less than 7.5μs
        t4: Low level width not less than 7.5μs
        """
        if d > 0:
            if self.enabled:
    ...
    def free_run(self, d):
        ...
        if self.timer_is_running:
            self.timer.deinit()
        if d != 0:
            # Limit frequency for DM332T timing constraints (~22μs per step minimum)
            effective_freq = min(self.steps_per_sec, 30000)
            self.timer.init(freq=effective_freq, callback=self._timer_callback)
    ...
    def enable(self, e):
        if self.en_pin:
            # t1: ENA must be ahead of DIR by at least 5μs
            self.en_pin.value(1)  # DM332T: ENA is active low
            if e:
                time.sleep_us(6)  # Ensure ENA is ahead of DIR
        self.enabled = e
        if not e:
            self.dir_value_func(not e)
